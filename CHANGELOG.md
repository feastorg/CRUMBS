...
- `crumbs_controller_read_expect()`: `crumbs_controller_read()` plus a check that the reply's `(type_id, opcode)` is the pair that was requested, returning `CRUMBS_RX_REPLY_MISMATCH` (`-7`) with `out_msg` still filled otherwise (`-7` is used by nothing else, so it is unambiguous through a getter). `expect_type_id == CRUMBS_TYPE_ID_ANY` skips the type half; the opcode is always checked (`0x00` is the version-info opcode, not a wildcard). The check previously existed only inside `CRUMBS_DEFINE_GET_OP` and, by hand, in every shipping getter. (#35)
