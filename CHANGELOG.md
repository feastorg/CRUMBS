...
- `CRUMBS_DEFINE_PAYLOAD(name, wire_bytes, FIELDS)` in `crumbs_ops.h`: one X-macro field list per payload layout generates the struct, the wire size, and `pack`/`unpack` functions for both sides, so a layout is stated once instead of as an append sequence on the controller and hand-written offsets on the peripheral. The build fails if the list does not sum to the declared length or exceeds 27 bytes. `unpack` checks the length once and reads at constant offsets; on AVR it compiles to the size of a careful hand-written unpack and about a third of the checked-helper idiom. (#36)
- `CRUMBS_DEFINE_FAMILY(PREFIX, type_id, OPS)`: declares a family's type and opcodes once and fails the build for a type outside `0x01`–`0xFF`, an opcode above `0xFF` or equal to `0xFE`, or two opcodes with one value (which `crumbs_register_handler()` would otherwise resolve by silently overwriting). `CRUMBS_STATIC_ASSERT` is public alongside. `tests/compile_fail/` holds one source per check; CTest builds each and requires the compiler to fail with that check's message. (#32)
...
- CRC-8 known-answer vectors: the suite now calls `crumbs_crc8()` directly and pins it to published CRC-8/SMBUS check values, including `crc8("123456789") == 0xF4`. Previously no test called it at all, so encode/decode self-consistency would have passed identically with any deterministic one-byte checksum, and a regenerated `crc8_nibble.c` with different parameters would have gone unnoticed. Regeneration of the pycrc-derived source is now gated in CI. (#21)
...
- `simple_linux_controller` assigned `argv[1]` to the device path before testing it for the `scan` keyword, so the scan mode the example advertises could never be reached. (#57)
...
- Documentation: every example README is rewritten to one shape — what it shows, its partner, addresses, the exact serial commands and printed lines, the build command — from the sketches rather than from memory, so the invented transcripts, the `SCAN`/`request=` gaps, the `SENSOR_COUNT` and Linux tool miscounts and the old ops-API call samples are gone; `examples/README.md` is the catalog and learning order and `docs/examples.md` is deleted. `README.md` is rewritten around two compiling quick-start snippets and the doc map; `docs/index.md` is the map. `advanced_controller`'s banner now shows integer examples, `request=` and `scan strict`, and three stale source comments are corrected. (#34, #52, #55)
- Documentation: `docs/create-a-family.md` is rewritten around `crumbs_ops.h` (466 → 239 lines): a `therm` family whose header, generated wrappers, peripheral and controller snippets all compile verbatim and were run end-to-end on a fake bus, with the parse-function signature shown, `crumbs_set_type_id()` and `crumbs_controller_read_expect()` in the flow, and the hand-written form for what the macros do not cover. The LHWIT family's home is now `examples/families_usage/lhwit_family/README.md` plus one README per device, each written from the code: real pins (the display's own header comment had the wrong ones), the servo's two channels and its speed unit, the calculator's unsigned results and `0xFFFFFFFF` on divide-by-zero, no `GET_SWEEP`, the mandatory `<type> <idx|@addr>` selector in the controllers, and no transcript that the code does not print. `docs/lhwit-family.md` is deleted. (#28, #50, #53, #54)
- Documentation: `docs/platform-setup.md` is rewritten (647 → 161 lines) with commands that run: CRUMBS is installed into the Arduino IDE as a folder (it is not in the Library Manager), PlatformIO uses `cameronbrooks11/CRUMBS` (not `feastorg/`), linux-wire must be *installed* (a build tree is not a prefix; the only prebuilt tarball is x86_64), and the Linux binaries are in `build-linux/`, not `build/`. `docs/platformio.md` is folded into it. `CONTRIBUTING.md` is rewritten (587 → 123 lines) around the checks that actually run — the doc gates, the CI job table, Conventional Commits, the release order — and absorbs the character-usage and Doxygen style guides, which are deleted. The last master/slave wording is gone, apart from the kernel's own "slave mode" name in the roadmap and a `/master/` branch URL. Em-dashes in code comments and test strings are replaced so the ASCII rule holds. (#31, #48, #49, #51)
- Documentation: `docs/api-reference.md` is now an index of every public symbol (one line each, grouped by task) and CI fails if a symbol is missing from it (`scripts/check_api_index.py`, reading the Doxygen XML); the header comments are the reference. Header comments that had drifted are corrected: the `CRUMBS_MAX_HANDLERS` memory figures (they counted one table where there are two), `crumbs_unregister_handler` / register return codes with tables compiled out, the handler `data` pointer (never `NULL`), the SET_REPLY receiver contract, `crumbs_encode_message` and `crc8` ownership, the two HAL `timeout_us` meanings, `crumbs_arduino_wire_write` and the Linux read return codes, and `nanosleep` vs `usleep`. Remaining master/slave wording in `src/` is gone. (#41, #42, #43, #44, #31)
...
- CI now gates what previously relied on review: Doxygen fails on an undocumented public symbol, `scripts/check_api_index.py` fails if a public symbol is missing from `docs/api-reference.md`, and Markdown relative links and anchors are checked. The build matrix gained ASan/UBSan, a no-handler-table configuration, and compilation of the Arduino sketches; the scanner tests now model foreign I2C devices so the CRC path is exercised rather than assumed. `TODO.md` is retired and `docs/roadmap.md` is the single home for deferred work. (#40, #56, #60)
- The Linux HAL requires **linux-wire 0.1.3 or newer** (`lw_probe`, `lw_set_target`); `find_package(linux_wire 0.1.3 ...)` enforces it at configure time, the installed `crumbsConfig.cmake` carries the same minimum through `find_dependency`, and both the CI and release workflows pin the 0.1.3 release tarball. `crumbs_linux_scan()` returns `-2` in non-strict mode if the adapter cannot perform an SMBus Quick Write, instead of reporting an empty bus. The seven `lw_set_slave` calls are now `lw_set_target`. (#65)
